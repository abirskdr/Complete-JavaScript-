...
//  !TOPIC 1: SCOPE
//  !TOPIC 2: Hoisting and TDZ
//    !2.1: var
//    !2.2: pitfall of hoisting (using var)
//  !TOPIC 3: "THIS" Keyword
//    !3.1:method borrowing
//    !3.2: copying function into a variable
//  !TOPIC 4: Regular Functions vs. Arrow Functions
//    !4.1: using arrow function inside object
//    !4.2: function inside of a method
//    !4.3: Arguments Keyword in functions
//  !TOPIC 5: Primitives Vs. Objects(Primitive vs. Reference Types)
//    !5.1: copying an object [Shallow copy]
//*******************************************************/
...
function calcAge(birthYear) {
  ...
  function printAge() {
    ...
    if (birthYear >= 1981 && birthYear <= 1996) {
      var millennial = true; //!var is not block scoped, it creates a property in the window object
      ...
      //!variables that are declared using const and let are block scoped
    ...
    //!if it was reassigned in a block that never got executed, we would not be able to get that value into this variable, like "never set"
  }
  printAge();
  return age;
}
...
if (!numProducts) deleteShoppingCart(); //?we get "All products deleted" even if numProducts is not 0
//!here numProducts is undefined which is also a falsy value so we get the above statement right
//!SO DON'T USE VAR TO DECLARE VARIABLES TO AVOID THESE TYPES OF BUGS
...
//f(); //!'this' will now be undefined, this happens because as we have copied the method from the abir object to a variable, it has now become a regular function call, 'this' is undefined in a regular function call.
...
const gabriel = {
  //!objects are not blocks, it DO NOT create its own scope, they cannot be accessed by the arrow function even by calling it with the method they are in as "this" used in arrow functions point to window scope, to access them the "this" should point the object they are in. [******Check the fact that they are in the global scope******].
  //!In JavaScript, key-value pairs in an object exist within the object’s own "namespace" or "container" — which is essentially the object itself. This means that each object holds its own properties internally, separate from any scope, whether global or local.
  fName: 'Gabriel',
  date: 25,
  calcAge: function () {
    console.log(this);
  ...
  greet: () => {
    ...
    console.log(`Hey ${this.fName}`); //?"this.fName" ?this gives us "undefined" as there is no fName till now in the window object
  },
};
...
const kyle = {
  ...
  kyleAge: function () {
    ...
    //! when we have a function inside of another function, and we are using this keyword inside of that function, if we are not returning that inner function, we would obviously have to call it inside of the outer function as a normal function call (as it is only available in that scope), and in that case we would have the "this" as undefined
    ...
    const arrowIsMillenial = () => {
      console.log(this);
      console.log(this.year >= 1981 && this.year <= 1996); //!as arrow functions do not have their own this keyword they use the this keyword of their parent scope (HERE THE PARENT SCOPE IS A METHOD)
    };
    arrowIsMillenial();
...
let oldLastName = lastName; //!as we do this the oldLastName also points to the address 0001
...
const jessica2 = {
  ...
  lastName: 'Williams',
  age: 27,
  family: ['Alice', 'Bob'], //!array is also an object (reference types)
};
...
//!PROBLEM: this technique only works for the first level, that is if there is an inner object (object inside of the object) the inner object will still be the same (still point to the sme place in the memory), that is why this is called a SHALLOW COPY.
...
//!Learn More about deep copy, write code for deep dopy, also learn about lodash's cloneDeep.
