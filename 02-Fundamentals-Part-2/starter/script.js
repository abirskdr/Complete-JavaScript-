...
// if (passTest) hasDriverLicense = true; //has a bug and that shows in the console, with strict mode --"hasDriversLicense" is the variable name and we have written "hasDriverLicense" which is not the right spelling, so in strict mode it gives an error.
...
const calcAge2 = function (birthyear) {
  //didnt give a name to the function (also called an anonymous function) so all of these is an expression the expression produces a value and stores it into calcAge2
  return 2037 - birthyear;
};
...
const newLength = friends.push("Cooper"); //adds elements at the end of the array, also returns value of the length of the new array
console.log(friends);
...
//remove elements
const popped = friends.pop(); //removes the last element of the array, returns the removed element,  if the method is applied on an empty array, it returns undefined
console.log(friends);
console.log(popped);

friends.shift(); // removes the first element of an array, returns the removed element, if the method is applied on an empty array, it returns undefined
console.log(friends);
...
console.log(friends.includes("Jay")); //checks and returns a boolean
friends.push(23);
...
console.log(
  `${paris.city} is the capital of ${paris.country} and has a population of ${
    paris.population
  } and my ${
    paris.casualFriends.length + paris.bestFriends.length
  } friends live there including ${paris.bestFriends[0]}.`
);
...
const jonas = {
  ...
  calcAgeOfJonas: function (birthYear) {
    //function expression (function is just a value and can be added to objects, even objects can be added to objects) only function expression is valid.
    return 2037 - birthYear;
  },
};
...
const niko = {
  ...
  getSummary: function () {
    ...
    this.summary = `${this.firstName} is a  ${this.calcAgeOfNiko()} year-old ${
      this.job
    }, and he ${canDrive} a driver's license`;
    return this.summary;
  },
};
...
for (let i = 0; i < numbers.length; i++) {
  if (numbers[i] == 44) {
    ...
    if (numbers[i] % 5 !== 0) continue; //skips to the next iteration if the number is not a multiple of 5.
