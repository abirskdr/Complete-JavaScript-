...
//   const request = new XMLHttpRequest(); //we create a new object request (everytime, for every country)
...
//     //we convert the responce into an actual JS object
//     const [data] = JSON.parse(this.responseText); //destructuring as well
//     console.log(data);

//     //maikng the tile with the data from the API
...
// //we might see the tiles in different order when we load it a number of times, as for each function call we have an AJAX call, all these AJAX calls will run in parallel and the AJAX call that recieves the data first is shown first, this shows the non-blocking behavior

// getCountryData('portugal');
// getCountryData('usa');

// //if we want to make the requests / recieve the data in a specific order we have to chain the requests (making the second request, only after the first request has finished)
...
// //previousy we made AJAX calls to fetch data, there were multiple AJAX calls made which ran parallely, and we could not control which one finished first, now we want to create a sequence of AJAX calls so that the second one finishes only after the first one has finished

// //here we take the country's data and look for the bodering countries in the "borders" property, what we want to do now is after the first AJAX call is completed, we will get the border and also render the neighbouring country, beside the original country, in this case the second AJAX call depends on the first call, without the first call we would not know what to fetch in the second AJAX call, what we need to do is a sequence of AJAX call
...
// //what if we want to make more requests in sequence (print neighbours of neighbours and so on), in that case we will end up with callbacks inside of callbacks inside of callbacks, (that would create a lot of nested callbacks) for such a structure and behaviour we have a special name "Callback Hell", "Callback Hell" is when we have a lot of nested callbacks, in order to execute asynchronous tasks in sequence (this happnes for all asynhronous tasks and not just AJAX calls)
...
// // ************************************************************* */
// // the problem with "Callback Hell" is that it makes our code look very messy and hard to maintain and diffiult to understand
// // ************************************************************* */
...
//       //                         (cannot yet look at the data yet, inorder to take a look at the data in the body, we need to call the "json()" method)*****
...
//   //the callback function will recieve one argument, once it is called by JS, and that one argument is the resulting value of the fulfilled promise (here)
// };
...
//     });
//   //************************************************************** */
//   //the then() always returns a promise, no matter if we actually return anything or not, however, if we do actually return a value, that will become the fulfilment value of the returned promise
//   //**************************************************************** */
// };
...
// //a only way in which a "fetch" promise is rejected is when the user looses his/her internet connection (we take this as the first error) [even when there is a status code of 404, the fetch promise still gets fulfilled]
...
//     }) //this "catch()" method will catch any error that occurs at any place in this chain, errors basically propagate down the chain untill they are caught, only when they are not caught, we get the "Uncaught error" messsage
...
//     }); // the callback function that is inside the "finally()" method will be called nomatter what happens with the promise (i.e. it is called nomatter the promise is fulfilled or rejected, the callback function will be called always)
//   //we use this method when we want to do something that we want to happen always nomatter the promise is fulfilled or rejected (like hiding the loading spinner, that we see when we load a data) (applications shows a spinner when an asynchronous operation starts, and hide it when the asynchronous operation is complete, this happens nomatter the operation was successsfull or not)
// };
...
// //1) to pass a second callback function into the "then()" method, the first callback function will always be going to be called for the fulfilled promise (for a successfull one), but we also have a second callback function that will be called when the promise will be rejected (the only argument that the second callback function recieves is the error thrown --abir)

// //then(response => response.json(),err => alert(err)) //second callback function that will be called when the promise is rejected

// //2) as we have to add the second callback function in the "then()" method for every fetch(), it can be annoying, so there is a method to handle error globally at one central place no matter where they occur in the chain by adding the "catch()" method at the end of the chain, and the callback function in the "catch()" method will be same as second callbackk function that we add in the "then()" method
...
//         //if we did not have this code block that throws error, we would have the error " Cannot read properties of undefined (reading 'flags'). Try again" as the fullfilled promise that would have been returned from here would go down, and somewhere we are trying to read the "flag" from the data that we recieved and the data that we recieved does not contain the "flag" property
//       }
...
// //////////////making the DRY version of the above code//////////////////
...
// //canot really do high precision  things using JS
...
//   // if (Math.random() >= 0.5) {
//   //   //we win the lottery (a fulfillled promise)
//   //   //in order to set the promise as fulfilled, we use the "resolve" function, (calling the "resolve" function will mark this promise as a fulfillled promise, or resolved promise)
...
// //////////////////Consuming the promise we built/////////////////////////
...
// /////////creating a fullfilled or rejected promise immediately//////////
...
// //we nake the "async" function by adding the "async" in front of the function
...
//   ///////////////we get the promise fron getPosition and consume it using the async, await/////////////////
...
//     renderCountry(data[0]);
//   } catch (err) {
//     //we handle the errors occuring in the "try" block, in the catch block
...
// // //the value that we return from the "async" function will becone the fulfilled value of the promise that is returned by the function, that is the value that we return from the "async" function (if theres no error)
...
// //the function we make will take in 3 contries and will log the capital cities of three countries as an array
...
// //******************************************************************* */
// //Whenever we need to do multiple asynchronous operations at the same time, and operations that dont depend on one another, we should always run them in parallel, using Promise.all()
// //******************************************************************* */
...
// //Promise.race takes an array of promises and returns a promise (this returned promise is the first settled promise from the array of promises that it recieves)
...
// (async function () {
//   //in the below, the three promises will race against each other (running parallely), and if the winning promise is then a fulfilled promise, then the fulfillment value of the winning promise is going to be the fulfillment value of race promise
...
// //Promise.allSettled necer short-circuits
...
// // //the result of Promise.any is always a fulfilled promise, inless ofcourse all of them reject
